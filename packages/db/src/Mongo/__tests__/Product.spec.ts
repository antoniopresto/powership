...
type GetFieldsUsedInIndexes<IndexItem, Kind> = Kind extends keyof IndexItem
  ? IndexItem[Kind] extends Array<infer F>
    ...
    [IndexName in IndexConfig['indexes'][number]['name']]: {
      [M in keyof IndexMethods<any, any>]: {
        [L in `${M}${Capitalize<IndexName>}`]: IndexMethods<
          Doc,
          IndexByName<IndexConfig['indexes'][number], IndexName>
        >[M];
      };
    };
...
type EntityLoader<
  Doc extends DocumentBase,
  IndexConfig extends AnyCollectionIndexConfig
> = EntityLoaders<Doc, IndexConfig>[keyof EntityLoaders<Doc, IndexConfig>];

const type = createType('User', { object: { name: 'string' } });
...
type Entity<Options extends EntityOptions> = Merge<
  {
    entity: Options['name'];
    indexes: Options['indexes'];
    type: Options['type'];
    parse: Options['type']['parse'];
  },
  EntityLoaders<
...
function createEntity<Options extends EntityOptions>(
  ...
  const { name, indexes, transporter: defaultTransporter } = options;
  ...
  indexConfig.indexes.forEach((index) => {
    ...
    transporterLoaderNames.forEach((method) => {
      ...
      _createLoader({ method, newMethodName: methodName, indexInfo });
    });
  });
...
describe('Product', () => {
  ...
  it('create entity', async () => {
    const type = createType('Product', {
      object: {
        ...
        hash: { ID: { autoCreate: true }, optional: true },
        SKU: { string: { min: 3 } },
      },
    });
    ...
    let product = await entity.findOneByStoreId({
    ...
    product = await entity.findOneByStoreId({
      filter: { storeId: 'store1' },
      dataloaderContext: {},
    });
  ...
  it('findByStore', async () => {
    //
  });

  it('findByStoreAndSKU', async () => {
    //
  });

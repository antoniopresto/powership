...
import { BacklandSessions } from '../src/backland-sessions';
...
describe('MongoSessions', () => {
  ...
  describe('#constructor', () => {
    it('should have default options', () => {
      const mongoSessions = new BacklandSessions({ database });
      expect((mongoSessions as any).options).toBeTruthy();
    });

    it('should overwrite options', () => {
      const mongoSessions = new BacklandSessions({
        database,
  ...
  describe('createSession', () => {
    ...
    it('using date provider on create session', async () => {
      const mongoSessions = new BacklandSessions({ database, dateProvider: () => new Date() });
      const sessionId = await mongoSessions.createSession(session.userId, 'token', {
        ip: session.ip,
        userAgent: session.userAgent,
      });
  ...
  describe('updateSession', () => {
    ...
    it('should not convert id', async () => {
      const mongoSessions = new BacklandSessions({ database, convertSessionIdToMongoObjectId: false });
      expect(
        async () =>

...
export type PowershipUtilsResolverInit = Infer<typeof JSONToSchemaOptions>;

export async function powershipUtilsResolver(
  input: PowershipUtilsResolverInit,
...
function renderForm(init: { json: string; url?: string }) {
  const { json, url = '/r/powership/toschema' } = init;
